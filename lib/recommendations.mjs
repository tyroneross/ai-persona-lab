...
import { assertIdSegment, pathSegmentProblem } from './idpath.mjs';
...
// Mirrors additionalProperties:false at the four levels the schema sets it. A
// record the CLI writes has to validate against the schema the CLI publishes,
// or every consumer told to validate a packet rejects our own output.
const PACKET_KEYS = ['schema_version', 'packet_id', 'run_id', 'lane', 'artifact', 'artifact_version',
  'outcome_frame', 'recommendations', 'open_questions', 'iteration', 'reminder', 'created_at'];
const FRAME_KEYS = ['question', 'answer', 'assumption'];
const RECOMMENDATION_KEYS = ['id', 'title', 'finding_ids', 'severity', 'provenance', 'adjudication',
  'recommendation', 'consumer', 'acceptance_check', 'status'];
const CONSUMER_KEYS = ['kind', 'name', 'note'];
const ITERATION_KEYS = ['round', 'prior_run_ids', 'trigger'];
// A packet_id becomes a filename. Path safety is enforced separately by
// assertIdSegment; this is the id SHAPE, so a saved packet cannot be named
// something a shell will fight over.
const PACKET_ID = /^[A-Za-z0-9][A-Za-z0-9_-]*$/;
const unknown = (obj, allowed, label) => Object.keys(obj).filter((k) => !allowed.includes(k))
  .map((k) => `${label} contains an unknown field: ${k}`);
...
export function validateRecommendationPacket(packet) {
  ...
  if (packet.packet_id !== undefined) {
    req(typeof packet.packet_id === 'string' && PACKET_ID.test(packet.packet_id),
      'packet_id must be alphanumeric with dashes or underscores — it becomes a filename');
  }
  errors.push(...unknown(packet, PACKET_KEYS, 'packet'));
  ...
  if (!Array.isArray(packet.outcome_frame) || packet.outcome_frame.length === 0) {
    ...
    packet.outcome_frame.forEach((entry, i) => {
      ...
      errors.push(...unknown(entry, FRAME_KEYS, `outcome_frame[${i}]`));
    });
  }
  ...
  if (!Array.isArray(packet.recommendations) || packet.recommendations.length === 0) {
    ...
    packet.recommendations.forEach((r, i) => {
      ...
      errors.push(...unknown(r, RECOMMENDATION_KEYS, `recommendations[${i}]`));
      if (!r.consumer || typeof r.consumer !== 'object' || Array.isArray(r.consumer)) {
        ...
        errors.push(...unknown(r.consumer, CONSUMER_KEYS, `recommendations[${i}].consumer`));
      }
  ...
  if (!packet.iteration || typeof packet.iteration !== 'object' || Array.isArray(packet.iteration)) {
    ...
    errors.push(...unknown(packet.iteration, ITERATION_KEYS, 'iteration'));
  }
...
/**
 * Everything `saveRecommendationPacket` does except the write: apply the
 * defaults the writer stamps, check the packet against the run, and validate.
 *
 * This exists so a dry run can PREDICT the write. Validating the caller's raw
 * input instead would reject every correctly-authored packet, because the author
 * is told to supply lane/outcome_frame/recommendations/iteration and the writer
 * supplies schema_version, artifact, artifact_version, reminder and created_at.
 * A gate that fails the inputs it documents is worse than no gate.
 *
 * Returns `{ ok, errors, packet, file }` and never throws on invalid input —
 * only on input that is not an object at all.
 */
export function prepareRecommendationPacket(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('recommendation packet must be a JSON object');
  const errors = [];
  const idProblem = pathSegmentProblem(input.run_id);
  if (idProblem) return { ok: false, errors: [`run_id ${idProblem}`], packet: null, file: null };
  const run = readRunRecord(input.run_id);
  if (!run) return { ok: false, errors: [`run not found: ${input.run_id}`], packet: null, file: null };
  if (run.status === 'abandoned') {
    errors.push(`run ${input.run_id} was abandoned — an unfinished panel does not issue work orders`);
  }

  const packet = {
    schema_version: RECOMMENDATION_PACKET_SCHEMA_VERSION,
    ...input,
    packet_id: input.packet_id || `packet_${randomUUID().replaceAll('-', '')}`,
    run_id: run.run_id,
  ...
  if (packet.artifact !== run.artifact?.slug) errors.push('artifact does not match the run');
  if (packet.artifact_version !== run.artifact?.version) errors.push('artifact_version does not match the run');
  errors.push(...validateRecommendationPacket(packet).errors);

  let file = null;
  const segmentProblem = pathSegmentProblem(packet.packet_id);
  if (segmentProblem) errors.push(`packet_id ${segmentProblem}`);
  else {
    file = safeStorePath('runs', run.run_id, 'recommendations', `${packet.packet_id}.json`);
    if (existsSync(file)) errors.push(`packet_id already recorded on this run: ${packet.packet_id}`);
  }
  return { ok: errors.length === 0, errors, packet, file };
}

export function saveRecommendationPacket(input) {
  const { ok, errors, packet, file } = prepareRecommendationPacket(input);
  if (!ok) throw new Error(`Invalid recommendation packet:\n- ${errors.join('\n- ')}`);
  mkdirSync(path.dirname(file), { recursive: true });
  // 'wx' is the append-only guarantee itself, not a formality: it is what makes a
  // second write to the same packet_id fail instead of erasing round one.
