...
import { assistsWith, informedBy, principlesForGuests } from './guests.mjs';
...
export function composePersona({ archetypes, specialties = [], task = '', name, guests = [] } = {}) {
  ...
  const guestSlugs = [...new Set(Array.isArray(guests) ? guests : String(guests || '').split(',').map(s => s.trim()).filter(Boolean))];
  // A guest restriction narrows the source pool before archetype relevance, so
  // "compose an engineer informed only by Will Larson" attaches Larson's
  // principles or none — never a silent substitution from another speaker.
  const pool = guestSlugs.length ? principlesForGuests(guestSlugs) : reviewedPrinciples();
  const relevant = pool.filter(e => e.archetypes.some(id => ids.includes(id)));
  ...
  const persona = withSaveDefaults({
    ...
    assists_with: assistsWith(selected, ids), informed_by: informedBy(selected),
  ...
  return { persona, principles: selected, evidence_gaps: gaps,
    ...(guestSlugs.length ? { guest_filter: guestSlugs } : {}),
    evidence_status: 'Reviewed source interpretations; synthetic persona and specialty application remain hypotheses.',
    source_note: 'Reviewed principles are reported viewpoints from podcast transcripts; not endorsement, not proof of competence.',
...
export function planConsultation(task, { specialties = [], archetypes = [], count = 5, mode = 'consultant', artifact, project } = {}) {
  ...
  const assignments = selected.map(id => {
    ...
    return { archetype_id: id, review_lens: base.lens, name: match?.name || composed.persona.name,
      ...
      principles: attached,
      // The orchestrator needs to see which reviewed principles inform each seat
      // before it dispatches, not after it reads the encounter.
      assists_with: match?.assists_with || assistsWith(attached, [id]),
      informed_by: match?.informed_by || informedBy(attached),
