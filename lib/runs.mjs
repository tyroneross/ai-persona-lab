...
export function validateRun(r) {
  ...
  if (r.study !== undefined) {
    const s = r.study;
    if (!s || typeof s !== "object" || Array.isArray(s)) errors.push("study must be an object");
    else {
      req(isStr(s.objective), "study.objective is required");
      req(Array.isArray(s.target_concepts) && s.target_concepts.length > 0 && s.target_concepts.every(isStr), "study.target_concepts must list nonempty concepts");
      req(Array.isArray(s.tasks) && s.tasks.length > 0 && s.tasks.every((x) => x && isStr(x.id) && isStr(x.instruction)), "study.tasks must contain id + instruction");
      req(Array.isArray(s.questions) && s.questions.length > 0, "study.questions must list at least one question");
      for (const [i, q] of (s.questions || []).entries()) {
        req(q && isStr(q.id) && isStr(q.prompt), `study.questions[${i}] requires id + prompt`);
        req(["recall", "relationship", "transfer", "boundary"].includes(q?.type), `study.questions[${i}].type is invalid`);
        req(Number.isFinite(q?.max_score) && q.max_score > 0, `study.questions[${i}].max_score must be positive`);
        req(Array.isArray(q?.expected_concepts) && q.expected_concepts.length > 0 && q.expected_concepts.every(isStr), `study.questions[${i}].expected_concepts must list nonempty concepts`);
        req(isStr(q?.rubric), `study.questions[${i}].rubric is required`);
      }
    }
  }
...
export function createRun({ request, artifact = {}, roster = [], level, notes, study }) {
...
export function renderReport(run) {
  ...
  for (const e of encs) {
    ...
    if (e.comprehension) {
      ...
      L.push("");
      L.push(`Comprehension: ${a ? `**${a.total_score}/${a.max_score} (${a.percent}%)**` : "awaiting independent assessment"}`);
      for (const answer of e.comprehension.answers || []) L.push(`- ${answer.question_id}: ${esc(answer.answer) || "*(no answer)*"}`);
      for (const item of a?.items || []) {
        L.push(`  - Score ${item.score}/${item.max_score}: ${esc(item.rationale)}`);
        if (item.misconceptions?.length) L.push(`    Misconceptions: ${item.misconceptions.map(esc).join("; ")}`);
      }
    }
    if (e.journey?.snapshots?.length) {
      L.push("");
      L.push("Visual checkpoints:");
      for (const shot of e.journey.snapshots) L.push(`- ${esc(shot.label)} — ${esc(shot.viewport)} · ${esc(shot.locator)}: ${esc(shot.observed)}`);
    }
