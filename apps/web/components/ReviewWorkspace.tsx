...
export default function ReviewWorkspace({ personas }: { personas: PersonaSummary[] }) {
  ...
  return (
    <div className="space-y-10">
      <section aria-labelledby="workspace-title" className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-2xl">
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted">Your next review</p>
          <h1 id="workspace-title" className="text-3xl font-semibold tracking-tight sm:text-4xl">Choose the perspectives.<br />Make the question precise.</h1>
        ...
        <span className="self-start rounded-full border border-line bg-surface px-3 py-1.5 text-xs text-muted">{personas.length} saved personas · Local library</span>
      </section>
      ...
      <section aria-labelledby="template-title" className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {REVIEW_PRESETS.map((item, index) => <button type="button" key={item.id} aria-pressed={presetId === item.id} onClick={() => { setPresetId(item.id); edited(); }} className={`rounded-xl border p-5 text-left transition ${presetId === item.id ? "border-ink bg-ink text-white shadow-sm" : "border-line bg-surface text-ink hover:border-line-strong"}`}>
            <span className={`text-xs ${presetId === item.id ? "text-slate-300" : "text-muted"}`}>0{index + 1} / Three review lenses</span>
            <span className="mt-4 block text-lg font-semibold">{item.title}</span>
            <span className={`mt-1 block text-sm ${presetId === item.id ? "text-slate-300" : "text-muted"}`}>{item.subtitle}</span>
          </button>)}
        </div>
      </section>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1.05fr_1fr]">
        ...
        <section aria-labelledby="panel-title" className="rounded-xl border border-line bg-surface p-5 sm:p-6">
