...
For a rendered navigation and comprehension study, create the run with `--study study.json`, then run `persona run journey <run_id> <persona_id> --viewports 390x844 --tools computer-use,screenshot`. Dispatch the generated `USER.md`, `SOUL.md`, `GOAL.md`, `CAPABILITIES.md`, and `packet.json`; never expose `ASSESSOR.json` to the persona. The host must attach and exercise real navigation, interaction, interface inspection, viewport, and snapshot capabilities; a declared tool name or Persona Lab UI is not proof. Require snapshots at consequential states and record visible placement in the encounter. Stop browsing before the persona answers. Score saved answers independently against `ASSESSOR.json`; keyword presence alone does not establish comprehension. Report this as synthetic synthesis evidence, not human validation or human recall.
